...
% action define
actInfo = rlNumericSpec([1 1], 'LowerLimit', -inf, 'UpperLimit', inf);
actInfo.Name = "K";
actInfo.Description = "feed back gain";
%% environment create

% env = rlFunctionEnv(obsInfo,actInfo,"myStepFunction","myResetFunction");
env = rlSimulinkEnv('simSystem','simSystem/RL Agent',obsInfo, actInfo);
...
agentOptions = rlDDPGAgentOptions('SampleTime', 0.1, 'DiscountFactor', 0.99,'ExperienceBufferLength', 1000);
agent = rlDDPGAgent(actor, critic, agentOptions);

%% open system
open_system("simSystem")
